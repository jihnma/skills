...
```sh
node ${CLAUDE_PLUGIN_ROOT}/skills/design-issue-to-pr/upload-attachments.mjs \
  --owner <O> --repo <R> --pr <N> \
...
- File paths should be the **per-component paths** (e.g. `.figma-react-cache/diff/Button/diff.png`), not the raw `.figma-react-cache/diff/diff.png` that figma-to-react writes. The wrapper moves files into per-component subdirs immediately after each `figma-to-react` invocation returns (see SKILL.md workflow step 5).
