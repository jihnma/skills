...
The wrapper auto-starts Storybook in the background if it isn't reachable, then polls until ready. This differs from `fe-design-implement`'s "never auto-start" rule — the wrapper exists for unattended issue-to-PR runs where blocking the user on `pnpm storybook &` defeats the purpose. The child skill still gets a reachable Storybook either way; only the lifecycle owner differs.
...
3. No signal → default `pnpm` (matches fe-design-implement's documented expectation).
