...
- File paths should be the **per-component paths** (e.g. `.fe-design-cache/diff/Button/diff.png`), not the raw `.fe-design-cache/diff/diff.png` that fe-design-create writes. The wrapper moves files into per-component subdirs immediately after each subagent returns (see SKILL.md workflow step 5).
