...
fe-design-* skills generate React components from Figma designs and verify the result matches visually. This file defines load-bearing terms specific to that workflow.
...
**figmaCategoryPath**:
Ordered path from Figma's page-level CANVAS down to a target node, with the page name dropped. Drives the **Storybook story title** so verify can locate the rendered component. Does **not** drive file system layout — that is inferred by AI from the project's existing component placement conventions.
