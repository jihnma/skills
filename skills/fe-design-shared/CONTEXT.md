...
**figmaCategoryPath**:
Ordered path from Figma's page-level CANVAS down to a target node, with the page name dropped. Drives the **Storybook story title** so fe-design-diff can locate the rendered component. Does **not** drive file system layout — that is inferred by AI from the project's existing component placement conventions.
