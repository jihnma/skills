...
- `fe-design-verify` exits with "out of scope" on stories that lack a Figma node mapping. It never falls back to comparing against a "closest" variant.
- `fe-design-implement` generates one story per Figma variant and binds each story's `parameters.design.url` to that specific variant node id. The parent `COMPONENT_SET` URL is not reused across variants.
